function [multi_channel_data, channel_info] = load_holdout_data(edfFilePath, desiredChannels)
...
for ch = 1:nChannels
    ...
    for e = 1:nEpochs
        s = (e-1)*samplesPerEpoch + 1;
        t = e*samplesPerEpoch;

        % Bound check
        if t > totalSamples
            validLen = max(0, totalSamples - s + 1);
            seg = zeros(1, samplesPerEpoch);
            if validLen > 0
                seg(1:validLen) = record(idx(ch), s:s+validLen-1);
            end
        else
            seg = record(idx(ch), s:t);
        end

        % store (left-aligned)
        multi_channel_data(e, ch, 1:samplesPerEpoch) = seg;
    end
end
