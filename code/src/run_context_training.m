...
test_idx = false(size(Y_all));
for i = 1:numel(classes)
    idxc = find(Y_all==classes(i));
    n = numel(idxc);
    ntest = max(1,round(n*cv_ratio));
    perm = idxc(randperm(n));
    test_idx(perm(1:ntest)) = true;
    train_idx(perm(ntest+1:end)) = true;
end
